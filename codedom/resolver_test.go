...
var _ = Describe("Resolver", func() {
	...
	Describe("Components", func() {
		...
		Describe("Headers", func() {
			Describe("integer", func() {
				BeforeEach(func() {
					spec = resolve("headers-integer.yaml")
					Expect(spec.Types).To(HaveLen(4))
				})
			...
			Describe("number", func() {
				BeforeEach(func() {
					spec = resolve("headers-number.yaml")
					Expect(spec.Types).To(HaveLen(4))
				})
			...
			Describe("string", func() {
				BeforeEach(func() {
					spec = resolve("headers-string.yaml")
					Expect(spec.Types).To(HaveLen(12))
				})
			...
			Describe("Enum", func() {
				...
				BeforeEach(func() {
					spec = resolve("headers-enum.yaml")
					Expect(spec.Types).To(HaveLen(2))
				})
			...
			Describe("Array", func() {
				BeforeEach(func() {
					spec = resolve("headers-array.yaml")
					Expect(spec.Types).To(HaveLen(2))
				})
			...
			Describe("Object", func() {
				...
				BeforeEach(func() {
					spec = resolve("headers-object.yaml")
					Expect(len(spec.Types)).To(Equal(5))
				})
